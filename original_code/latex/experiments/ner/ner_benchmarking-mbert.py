...
MODEL_PATHS = {
    "modernbert": {
        "model": "/gpfs/helios/home/abdelrah/ModernBERT/Training2/output/checkpoint_step_13000/",
        "tokenizer": "/gpfs/helios/home/abdelrah/ModernBERT/Training2/Tokenizer",
    },
    "arabert": {
...
DEFAULT_DATA_DIR = "/gpfs/helios/home/abdelrah/ModernBERT/Benchmarking/Data/"
...
def create_compute_ner_metrics(id2label):
    ...
    Returns:
        Function that computes detailed NER metrics including per-label analysis
    """
    def compute_ner_metrics(pred):
        ...
        Returns:
            Dictionary containing overall and per-label metrics, plus confusion matrix
        """
        ...
        true_predictions = [
            [p for (p, lbl) in zip(prediction, label) if lbl != TOKEN_IGNORED_LABEL]
            for prediction, label in zip(predictions, labels)
        ]
        true_labels = [
            [lbl for lbl in label if lbl != TOKEN_IGNORED_LABEL]
            for label in labels
        ]

        # Flatten the predictions and labels for metric calculation
        flat_predictions = [p for pred in true_predictions for p in pred]
        flat_labels = [lbl for lab in true_labels for lbl in lab]
        ...
        per_label_report = classification_report(
            ...
            output_dict=True,
            zero_division=0,
            target_names=[id2label.get(i, f"LABEL_{i}") for i in sorted(set(flat_labels + flat_predictions))]
        )
        ...
        logging.info("Per-label NER metrics:")
        ...
        logging.info(f"Overall Macro-F1: {macro_f1:.4f}")
        logging.info(f"Overall Weighted-F1: {weighted_f1:.4f}")
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="modernbert",
        choices=["modernbert", "arabert", "mbert"],
